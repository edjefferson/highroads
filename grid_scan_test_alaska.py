...
def extract_img_data(image_file):
    ...
    zip_ref.extractall('temp_dir_alaska')
    zip_ref.close()
    print("zip extracted")

    file = glob.glob('temp_dir_alaska/*.img')[0]
    shutil.copy(file,"output_alaska.img")
    print("img extracted")

    shutil.rmtree('temp_dir_alaska')
    print("temp files deleted")



def get_elevation_array():
    img=gdal.Open("output_alaska.img")
...
def process_grid_square(image_file,road_file,bounding_box_string):
    ...
    for state in df.state.unique():
        ...
        #print(sorted)
        sorted.head(100).to_csv("final_alaska/%s_high.csv" % (state), mode='a', header=False)
        sorted.tail(100).to_csv("final_alaska/%s_low.csv" % (state), mode='a', header=False)
